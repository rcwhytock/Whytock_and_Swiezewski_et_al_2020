...
for (i in 1:length(stations)) {
  ...
  for (k in 1:length(thresholds)) {
    ...
    outFrame$observedRichness[k] <-
      length(unique(mergeLabels_subset[which(mergeLabels_subset$Station == stations[i] & mergeLabels_subset$topLabelScore >= thresholds[k]), "Species_Appsilon"]))
...
pdf("../figures/speciesRichness_thresholds_subsetManualAnnots.pdf", width = 11, height = 8.5)
...
# Calculate baseline activity model for each focal species using subset dataset for each threshold and manual labels
manualActivityModels <- vector("list", length = length(thresholds)*length(focalSpecies))
...
for(i in 1:length(thresholds)) {
  ...
  for (j in 1:length(focalSpecies)) {
    newDatSpecies <- newDat[newDat$Species_Appsilon == focalSpecies[j], ]
    
    if (nrow(newDatSpecies) > 10) {
      TimesML <- as.POSIXct(ymd_hms(newDatSpecies[newDatSpecies$Species_Appsilon == focalSpecies[j], "date_time"]))
    ...
    manualActivityModels[[index]] <- modML
    index <- index +1
...
for(i in 1:length(thresholds)) {
  ...
  for (j in 1:length(focalSpecies)) {
    if (!is.na(MLActivityModels[[index]])) {
      ...
      mod2 <- manualActivityModels[[index]]
...
compareResDF <- compareResDF[-1,] # remove the extra line from beginning
...
write.csv(compareResDF, "../results/compareActivity_subsetManualAnnots.csv", row.names = F)
...
# Differences become non significant after 0.5
pdf("../figures/activityPattern_point5_subsetManualAnnots.pdf", width = 6, height = 6)
...
index <- 21:24
  for(i in 1:length(focalSpecies)){
    ...
    plot(manualActivityModels[[index[i]]], 
         add = TRUE,
